#include "arduino.h"
/*
  chirp signal with DDS sin wave generator
...
void loop()
{
  ...
  while(1)
  {
	int frequency=map(n,0,duration_sec*1000000UL/samplingTime_us,fstart_hz,fstop_hz);
	n++;
	if(n>duration_sec*1000000UL/samplingTime_us)
	{
		n=0;
		digitalToggle(ledPin);
	}

	int phaseDelta=frequency*samplingTime_us*65536/1000000UL;
